"use strict";

const SUCCESS = require("../libs/app-codes").SUCCESS;
const INTERNAL_SERVER_ERROR =
  require("../libs/app-codes").INTERNAL_SERVER_ERROR;
const AppError = require("../libs/app-error").AppError;

function sendSuccessResponse(res) {
  const successResponse = {
    status_code: SUCCESS.httpCode,
    message: SUCCESS.details,
    timestamp: Math.round(Date.now() / 100),
  };

  if (res.data) {
    successResponse.data = res.data;
  }

  res.status(SUCCESS.httpCode).json(successResponse);
}

function sendErrorResponse(err, res) {
  err = Object.assign(
    {
      httpCode: INTERNAL_SERVER_ERROR.httpCode,
      description: INTERNAL_SERVER_ERROR.details,
    },
    err
  );

  return res.status(err.httpCode).json({
    status_code: err.httpCode,
    message: err.description,
    timestamp: Math.round(Date.now() / 100),
  });
}

module.exports = (err, req, res) => {
  if (!res || !res.json || typeof res.json !== "function") {
    throw new AppError(INTERNAL_SERVER_ERROR);
  }

  err ? sendErrorResponse(err, res) : sendSuccessResponse(res);
};
